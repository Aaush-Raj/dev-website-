 ...
 * along the foot.
 *
 * ONLY THE PHOTOGRAPH SHIPS. The design pack also supplies the cards and their
 * connectors as a 363KB PNG and the foot rail as a 164KB one; both are pure
 * interface, so they are drawn here instead — the text stays selectable,
 * translatable and legible to screen readers, and the strokes stay crisp at
 * any density.
 *
 * THE CONNECTORS ARE CSS, NOT SVG. They are short dashed runs between card
 * edges, and every attempt on this build at drawing that shape in a stretched
 * viewBox came out wrong — `preserveAspectRatio="none"` skews the dashes and
 * flattens the curve. A positioned element with a repeating-gradient dash has
 * no coordinate space to distort.
 *
 * READING ORDER IS THE CONNECTOR ORDER. The cards are listed top-right, then
 * bottom-left, then centre, because that is how the chain runs — not how they
 * stack visually. Sorting them by position would break the sequence the
 * connectors draw.
 */

const { reality } = onboarding;
...
export function OnboardingReality() {
    ...
    >
      ...
      <div aria-hidden="true" className="absolute inset-0 -z-10">
        ...
        {/*
          ...
          against a scene whose brightest area — the lit building — sits behind
          it; the second sinks the foot into the section ground so the
          consequence rail sits on flat colour rather than on tarmac.
        */}
        <div
          className={cn(
            "absolute inset-0",
            "bg-[linear-gradient(to_right,rgb(13_17_31/0.95)_0%,rgb(13_17_31/0.8)_34%,rgb(13_17_31/0.42)_58%,rgb(13_17_31/0.55)_100%)]",
          )}
        />
        <div
          className={cn(
            "absolute inset-x-0 bottom-0 h-[38%]",
            "bg-[linear-gradient(to_bottom,transparent,rgb(21_28_47/0.96)_76%,#151c2f_100%)]",
          )}
        />
      </div>
      ...
      {/*
        ...
        under it — laying the three out in a block after the copy pushed that
        card into the lower half, because its measured 5% offset was then 5% of
        a box starting at 43.5% of the section rather than of the section
        itself. The percentages below are the design's own, so the box has to
        span the same frame they were measured against.

        `xl` only: below that the cards stack, since three floating over a scene
        this size would overlap and the dashes would be too short to follow.
      */}
      <Uncopyable
        className={cn(
          "pointer-events-none absolute inset-x-0 hidden xl:block",
          /*
            The band opens level with the headline — the role-readiness card
            sits beside it in the design — and closes above the foot rail.

            The top inset is what keeps the lower two cards clear of the
            description. Tuning it alone was not enough: narrowing the
            description to make room instead made it TALLER (four lines rather
            than three), so the measure is set wide enough to hold three and the
            band starts below where it ends.
          */
          "top-[20%] bottom-[17%]",
        )}
      >
      ...
      {/* ====================== Consequence rail ====================== */}
      <div className="relative border-t border-white/10 bg-[#151c2f]">
        <Container width="hero">
          <ul
            className={cn(
              "grid gap-y-6 py-7",
 ...
 *
 * Percentages are of the card box, measured from the design. Each run starts at
 * one card's edge and ends at the next card's, following the reading order the
 * cards are listed in.
 */
function Connectors({ reduce }: { reduce: boolean }) {
  ...
  return (
    <div aria-hidden="true" className="pointer-events-none absolute inset-0">
      {/* ----- Generic card → questions card: a short run rightward ----- */}
      <motion.span
        {...draw(0.62)}
        className="absolute h-px origin-left"
        style={{
          // Cyan card's right edge (28.7%) to the red card's left (33.9%).
          left: "28.7%",
          top: "62%",
          width: "5.2%",
          backgroundImage:
      ...
      {/* --- Questions card → readiness card: up and to the right ------- */}
      <motion.span
        {...draw(0.74)}
        className="absolute h-px origin-left"
        style={{
          /*
            Red card's top-right corner to the amber card's bottom-left, both
            read off the rendered band: (55.6%, 55%) to (68.3%, 19.1%). That is
            263px at -50.4 degrees, which is 19.9% of the band's width. Guessing
            the angle put this run across the subject's chest instead of between
            the two corners.
          */
          left: "55.6%",
          top: "55%",
...
/** One size fits all — a document with a folded corner. */
...
/** Knowledge support — two speech bubbles, the front one carrying a question. */
function QuestionIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" className={className}>
      {/* The bubble behind, offset up and right so the pair reads as two. */}
      <path
        d="M9.5 4h9A1.5 1.5 0 0 1 20 5.5v5A1.5 1.5 0 0 1 18.5 12H17"
        {...stroke}
      />
      <path
        d="M12.5 8h-9A1.5 1.5 0 0 0 2 9.5v6A1.5 1.5 0 0 0 3.5 17H5v3l3.5-3h4A1.5 1.5 0 0 0 14 15.5v-6A1.5 1.5 0 0 0 12.5 8Z"
        {...stroke}
      />
      {/* The mark itself: a hook and a dot, not a glyph — it stays legible small. */}
      <path d="M6.6 11.3a1.5 1.5 0 0 1 2.9.5c0 1-1.5 1.2-1.5 2.2" {...stroke} />
      <circle cx="8" cy="16" r="0.85" fill="currentColor" />
    </svg>
  );
}
...
/** Readiness unclear — stacked discs, as the design's rail draws them. */
